
library(lme4)
library("rstudioapi")    
library(rstan)
library(reshape2)

rm(list=ls())
options(mc.cores = parallel::detectCores())

# Get current path
script_path = rstudioapi::getActiveDocumentContext()$path
pathSplit=strsplit(script_path, "/")
pathSplit=pathSplit[[1]]
main_path=paste0(pathSplit[1:(length(pathSplit)-2)],"/",collapse="")
analysis_path=paste0(pathSplit[1:(length(pathSplit)-1)],"/",collapse="")
setwd(analysis_path)

## Sample
path=paste0(main_path,"/Output/")
subjects=c(101:110,112:121); # valid subjects
# Not really excluded:
# 111 - Eyetracker crashed code during the experiment. 

filelist=c()
for (s in subjects){
  filelist=c(filelist,Sys.glob(paste(path, "BM_",s,"_CAT_*.txt",sep="")))
}
CAT_data=c()
for (f in filelist){
  CAT_data=rbind(CAT_data,read.table(f,header=T,na.strings=c(999,999000)))
}

CAT_data$sub_i = as.numeric(factor(CAT_data$Subject))
CAT_data$Contingency2 = factor(CAT_data$Contingency, labels =  c("0% Contingency", "50% Contingency","100% Contingency"))
CAT_data$Run2 = as.factor(CAT_data$Run)
CAT_data$Response = 1 - is.na(CAT_data$RT)
CAT_data$RT_eff = CAT_data$RT - CAT_data$CueOnset

CAT_data_GO = subset(CAT_data, Go==1 & Response == 1 & RT>=250)

ggplot(data = CAT_data_GO, aes(x = Run, y = RT, color = Contingency2)) +
  geom_point(position = 'jitter', alpha = 0.3) +
  geom_smooth(method = 'lm')

ggplot(data = CAT_data_GO) +
  facet_grid(. ~ Contingency2) +
  geom_density(aes(x = RT, y = ..density.., color = Run2)) +
labs(color = "Training Run", x = "Reaction time (ms)") +
  xlim(c(0,1500))+ ylim(c(0,.0063)) + geom_vline(xintercept = 850, linetype =2) +
  scale_y_continuous(expand = c(0,0)) + 
  theme_bw() +
  theme(text = element_text(size=18)) + 
  annotate(geom="text", x=550, y=.006, label="Cue Onset")


for (i in 1:20){
p = ggplot(data = subset(CAT_data_GO, Contingency==1 & Run <=i)) +
  geom_density(aes(x = RT, y = ..density.., color = Run2))+
  xlim(c(0,1500))+ ylim(c(0,.0063)) + geom_vline(xintercept = 850, linetype =2)+
  theme(text = element_text(size=20)) +
 labs(color = "Training Run", x = "Reaction time (ms)") +
  annotate(geom="text", x=800, y=.006, label="Cue Onset")

# png(filename=paste0(i,".png"))
# plot(p)
dev.off()
}

mean_RT = as.data.frame(tapply(CAT_data_GO$RT, list(CAT_data_GO$Run,CAT_data_GO$Contingency), mean, na.rm = T))
mean_RT$Run = as.numeric(rownames(mean_RT))
mean_RT2 = melt(data = mean_RT, id.vars = "Run", variable.name = "Contingency", value.name = "MeanRT")
ggplot(data = mean_RT2, aes(x = Run, y = MeanRT, fill = Contingency)) +
  geom_bar(stat = 'identity', position = 'dodge') 

ggplot(data = mean_RT2, aes(x = Run, y = MeanRT - 850, fill = Contingency)) +
  geom_bar(stat = 'identity', position = 'dodge') 

# Logistic regression plot
CAT_data_GO$anticipatoty = as.numeric(CAT_data_GO$RT<CAT_data_GO$CueOnset)
ggplot(CAT_data_GO, aes(x=Run, y=anticipatoty, color = Contingency2)) + geom_jitter(width = 0.5, height = 0.01, alpha=0.1) + 
  stat_smooth(method="glm", method.args=list(family="binomial"), se=FALSE)


# STAN model1 - only 100% contingency: data organization ====
stan_file = "training_mixture_100%_contingency.stan"

subs2test = 1:1000 # set to examine less participants for speed
dat = subset(CAT_data_GO, sub_i %in% subs2test & Contingency == 1)
N_trials_per_sub = as.vector(tapply(dat$RT, list(dat$sub_i), length))
N_trials_max = max(N_trials_per_sub)
N_runs = max(dat$Run)
N_subjects = nlevels(factor(dat$sub_i))

RT_table = matrix(data = 999000,nrow = N_trials_max,ncol = N_subjects)
Run_table = RT_table
N_trials_valid = c()
for (n in 1:N_subjects){
  # find values
  RT_tmp = dat$RT[dat$sub_i == n]
  Run_tmp = dat$Run[dat$sub_i == n]
  # scale values
  Run_tmp = (Run_tmp-1)/max(Run_tmp-1)
  # organize together
  N_trials_valid[n] = sum(dat$sub_i == n)
  RT_table[1:N_trials_valid[n],n] = RT_tmp
  Run_table[1:N_trials_valid[n],n] = Run_tmp  
}

stan_data = list(
  Run = Run_table,
  Cue = 850,
  theta_b0 = -3.1, # fix the proportion of early onset at pnorm(theta_b0) at the first run
  RT = RT_table,
  N_trials = N_trials_max,
  N_trials_valid = N_trials_valid,
  N_runs = N_runs,
  N_subjects = N_subjects
)

# STAN model1 - only 100% contingency: write model ----

write("// Stan model mixture gaus model - training with random theta
      data {
      int N_subjects;
      int N_trials;
      int N_trials_valid[N_subjects];
      real Cue;
      real theta_b0;
      real RT[N_trials, N_subjects];
      real Run[N_trials, N_subjects];
      }
      
      parameters {
      // Fixed effect: mu1(Anticipatory), mu2(Cue dependent), theta1, theta2, sigma_e1, sigma_e2
      real<lower=0, upper=Cue+100> mu1_fix; // Anticipatory mean
      real<lower=Cue> mu2_fix; // Cue dependent mean
      real theta_fix[1]; // theta0 + slope (for run)
      real<lower=0> sigma_e[2]; // error terms
      
      // SD for random effect: mu1, theta1, theta2
      real<lower=0> mu1_sd[1]; // SD of the random effects around the fixed mean 
      real<lower=0> theta_sd[1]; // SD of the random effects around the fixed thetas
      
      // Random effect parameter: mu1, theta1, theta2
      real <upper=Cue+100> mu1_rand[N_subjects]; // random intercept for early onest Gaussian's mean
      // real theta_b0_rand[N_subjects]; // random intercept for theta - fix at -2.5
      real theta_b1_rand[N_subjects]; // random intercept for theta
      }
      
      
      model {
      //priors
      mu1_fix ~ normal(500,500);
      mu2_fix ~ normal(1000,500);
      theta_fix ~ normal(0,0.7);
      sigma_e ~ normal(0,1000);
      theta_sd ~ normal(0.7,0.7);
      mu1_sd ~ normal(0,150);
      
      
      //likelihood
      for (s in 1:N_subjects){
      mu1_rand[s] ~ normal(mu1_fix,mu1_sd);
      // theta_b0_rand[s] ~ normal(theta_fix[1],theta_sd[1]); fix at -2.5
      theta_b1_rand[s] ~ normal(theta_fix[1],theta_sd[1]);
      
      for (t in 1:N_trials_valid[s]){
      target += log_mix(Phi_approx(theta_b0 + theta_b1_rand[s] * Run[t,s]),
      normal_lpdf(RT[t,s] | mu1_rand[s] , sigma_e[1]),
      normal_lpdf(RT[t,s] | mu2_fix, sigma_e[2]));
      }
      }
      }", 
      stan_file)

# STAN model1 - only 100% contingency: run ----

# fit_stan1 <- stan(file = stan_file, data = stan_data, iter = 2000, chains = 4,seed = 1, control = list(adapt_delta =.8))
# save(list=c("stan_data","fit_stan1", file = paste0(analysis_path,"fit_stan1.Rda"))
load(file = paste0(analysis_path,"fit_stan1.Rda"))

print(fit_stan1)
traceplot(fit_stan1, inc_warmup = FALSE)

# STAN model1 - only 100% contingency: simulate posterior data ----
params = c()
for (chain in 1:4){
  params = rbind(params,as.data.frame(extract(fit_stan1, permuted=FALSE, inc_warmup = FALSE)[,chain,]))
}
chains_length <- length(params$lp__)
N_subjects = stan_data$N_subjects
N_trials = stan_data$N_trials

simulations = data.frame(Sub = rep(1:N_subjects,rep(N_trials,N_subjects)),
                         Trial = rep(1:N_trials,N_subjects),
                         Run = as.vector(stan_data$Run),
                         Cue = as.vector(stan_data$Cue), 
                         RT = as.vector(stan_data$RT)
)

simulations = simulations[simulations$RT<=10000,]
n_sims = nrow(simulations)
simulations$sim_i = sample(1:chains_length,n_sims,replace = TRUE)
simulations$theta_b0 = stan_data$theta_b0
simulations$theta_b1 = NA
for (sub_i in 1:N_subjects){
  theta_b1_varname = paste0("theta_b1_rand[",sub_i,"]")
  mu1_b0_varname = paste0("mu1_rand[",sub_i,"]")
  mu2_b0_varname = "mu2_fix"
  
  simulations$theta_b1[simulations$Sub == sub_i] = params[simulations$sim_i[simulations$Sub == sub_i],theta_b1_varname]
  simulations$mu_1[simulations$Sub == sub_i] = params[simulations$sim_i[simulations$Sub == sub_i],mu1_b0_varname]
  simulations$mu_2[simulations$Sub == sub_i] = params[simulations$sim_i[simulations$Sub == sub_i],mu2_b0_varname]
  
  cat("\rprogress:",round(100*sub_i/N_subjects),"%")
  flush.console() 
}

simulations$theta = simulations$theta_b0 + simulations$theta_b1*simulations$Run
simulations$p = pnorm(simulations$theta)
simulations$mu_2 = params$mu2_fix[simulations$sim_i]
simulations$sigma_1 = params$`sigma_e[1]`[simulations$sim_i]
simulations$sigma_2 = params$`sigma_e[2]`[simulations$sim_i]
simulations$selection = rbinom(n_sims,1,simulations$p)
simulations$RT_simulation = rnorm(n_sims, simulations$mu_1 ,simulations$sigma_1) * simulations$selection +
  rnorm(n_sims, simulations$mu_2 ,simulations$sigma_2) * (1-simulations$selection)
hist(simulations$RT_simulation - simulations$Cue,100)
plot(simulations$RT_simulation ,simulations$RT )

simulations$RT_simulation_eff = simulations$RT_simulation - simulations$Cue
simulations$RT_eff = simulations$RT - simulations$Cue
simulations$Run2 = as.factor(simulations$Run*19 + 1)
simulations$selection_fac= as.factor(simulations$selection)

ggplot(data = simulations, aes(x = RT_simulation, y = RT,  color = Run)) +
  geom_point(alpha = 0.5) +
  geom_abline() 

# Actual Data
ggplot(data = simulations) +
  geom_density(aes(x =  RT,y = ..density.., color = Run2)) 
# Posterior Distribution
ggplot(data = simulations) +
  geom_density(aes(x =  RT_simulation,y = ..density.., color = Run2)) 

ggplot(data = simulations) + geom_density(aes(x = RT, y = ..density..,color = 'Original Data')) + 
  geom_density(aes(x = RT_simulation, y = ..density..,color = 'Posterior Simulation'))

# Simulated posterior - split by Gaussians: count
ggplot() +
  geom_density(data = subset(simulations, selection ==0), aes(x =  RT_simulation,y = ..count.., color = Run2)) + 
  geom_density(data = subset(simulations, selection ==1), aes(x =  RT_simulation,y = ..count.., color = Run2))

ggplot() +
  geom_density(data = subset(simulations, selection ==0), aes(x =  RT_eff,y = ..count.., color = Run2)) 
ggplot() +
  geom_density(data = subset(simulations, selection ==1), aes(x =  RT_eff,y = ..count.., color = Run2))

### STAN model1 - only 100% contingency: correlation with choice ----

model_summary = as.data.frame(summary(fit_stan1)$summary)
theta_mean = model_summary$mean[grep("theta_b1_rand",row.names(model_summary))]
mu1_mean = model_summary$mean[grep("mu1_rand",row.names(model_summary))]

filelist=c()
for (s in subjects){
  filelist=c(filelist,Sys.glob(paste(path, "BM_",s,"_Probe_*.txt",sep="")))
}
Probe_data=c()
for (f in filelist){
  Probe_data=rbind(Probe_data,read.table(f,header=T,na.strings=c(999,999000)))
}

probe_dat = subset(Probe_data,Contingency ==1 )
probe_dat$sub_i2 = as.numeric(factor(probe_dat$subjectID))
probe_mean = tapply(probe_dat$Outcome, probe_dat$sub_i2, mean, na.rm = T)
chose_go = tapply(probe_dat$Outcome==1, probe_dat$sub_i2, sum, na.rm = T)
chose_nogo = tapply(probe_dat$Outcome==0, probe_dat$sub_i2, sum, na.rm = T)

DM_dat = data.frame(theta = theta_mean, mu1 = mu1_mean, probe_effect = probe_mean,
                    chose_go = chose_go, chose_nogo = chose_nogo)
ggplot(data = DM_dat[,], aes(x = theta, y = probe_effect)) +
  labs(x = expression(paste(theta," slope")), y = 'proportion of trials Go stimuli were chosen') +
  geom_smooth(method = 'lm', se= FALSE) +
  geom_smooth(method = 'glm', color = "red", method.args = list(family = "binomial"), se= FALSE) +
  scale_y_continuous(limits = c(0,1), breaks=seq(0, 1, 0.1),expand = c(0,0),  name = "Proportion of trials Go stimuli were chosen") +
  geom_abline(intercept = (0.5),slope=0,linetype =2) + # chace level 50% reference line  
  geom_point()
summary(lm.beta(lm(probe_effect ~ theta, data = DM_dat)))
summary(glm(cbind(chose_go,chose_nogo) ~ 1 + theta, data = DM_dat, family = "binomial"))
# STAN model2 - both contingency levels: data organization ====
stan_file = "training_mixture_both_contingency_levels.stan"

subs2test = 1:1000 # set to examine less participants for speed
dat = subset(CAT_data_GO, sub_i %in% subs2test)
N_trials_per_sub = as.vector(tapply(dat$RT, list(dat$sub_i), length))
N_trials_max = max(N_trials_per_sub)
N_runs = max(dat$Run)
N_subjects = nlevels(factor(dat$sub_i))

RT_table = matrix(data = 999000,nrow = N_trials_max,ncol = N_subjects)
Run_table = RT_table
Contingency_table = RT_table
N_trials_valid = c()
for (n in 1:N_subjects){
  # find values
  RT_tmp = dat$RT[dat$sub_i == n]
  Run_tmp = dat$Run[dat$sub_i == n]
  Contingency_tmp = dat$Contingency[dat$sub_i == n]
  # scale values
  Run_tmp = (Run_tmp-min(Run_tmp))/max(Run_tmp-min(Run_tmp))
  Contingency_tmp = (Contingency_tmp-min(Contingency_tmp))/max(Contingency_tmp-min(Contingency_tmp))
  # organize together
  N_trials_valid[n] = sum(dat$sub_i == n)
  RT_table[1:N_trials_valid[n],n] = RT_tmp
  Run_table[1:N_trials_valid[n],n] = Run_tmp 
  Contingency_table[1:N_trials_valid[n],n] = Contingency_tmp 
}

stan_data = list(
  Run = Run_table,
  Cue = 850,
  theta_b0 = -3.1, # fix the proportion of early onset at pnorm(theta_b0) at the first run
  RT = RT_table,
  Contingency = Contingency_table,
  N_trials = N_trials_max,
  N_trials_valid = N_trials_valid,
  N_runs = N_runs,
  N_subjects = N_subjects
)

# STAN model2 - both contingency levels: write model ----

write("// Stan model mixture gaus model - training with random theta
      data {
      int N_subjects;
      int N_trials;
      int N_trials_valid[N_subjects];
      real Cue;
      real theta_b0;
      real RT[N_trials, N_subjects];
      real Run[N_trials, N_subjects];
      real Contingency[N_trials, N_subjects];
      }
      
      parameters {
      // Fixed effect: mu1(Anticipatory), mu2(Cue dependent), theta1(100% contingency), theta2(50% contingency), sigma_e1, sigma_e2
      real<lower=0, upper=Cue+100> mu1_fix; // Anticipatory mean
      real<lower=Cue> mu2_fix; // Cue dependent mean
      real theta_fix[2]; // theta0 + slope (for run)
      real<lower=0> sigma_e[2]; // error terms
      
      // SD for random effect: mu1, theta1, theta2
      real<lower=0> mu1_sd[1]; // SD of the random effects around the fixed mean 
      real<lower=0> theta_sd[2]; // SD of the random effects around the fixed thetas
      
      // Random effect parameter: mu1, theta1, theta2
      real <upper=Cue+100> mu1_rand[N_subjects]; // random intercept for early onest Gaussian's mean
      // real theta_b0_rand[N_subjects]; // random intercept for theta - fix at -2.5
      real theta_b1_rand[N_subjects]; // random intercept for theta
      real theta_b2_rand[N_subjects]; // random intercept for theta

      }
      
      
      model {
      //priors
      mu1_fix ~ normal(500,500);
      mu2_fix ~ normal(1000,500);
      theta_fix ~ normal(0,0.7);
      sigma_e ~ normal(0,1000);
      theta_sd ~ normal(0.7,0.7);
      mu1_sd ~ normal(0,150);
      
      
      //likelihood
      for (s in 1:N_subjects){
      mu1_rand[s] ~ normal(mu1_fix,mu1_sd);
      // theta_b0_rand[s] ~ normal(theta_fix[1],theta_sd[1]); fix at -2.5
      theta_b1_rand[s] ~ normal(theta_fix[1],theta_sd[1]); // 100% contingency
      theta_b2_rand[s] ~ normal(theta_fix[2],theta_sd[2]); // 50% contingency
      
      for (t in 1:N_trials_valid[s]){
      target += log_mix(Phi_approx(theta_b0 + (Contingency[t,s]*theta_b1_rand[s] + (1-Contingency[t,s])*theta_b2_rand[s]) * Run[t,s]),
      normal_lpdf(RT[t,s] | mu1_rand[s] , sigma_e[1]),
      normal_lpdf(RT[t,s] | mu2_fix, sigma_e[2]));
      }
      }
      }", 
      stan_file)

# STAN model2 - both contingency levels: run ----

#fit_stan2 <- stan(file = stan_file, data = stan_data, iter = 2000, chains = 4,seed = 123, control = list(adapt_delta =.8))
# save(list=c("stan_data","fit_stan2"), file = paste0(analysis_path,"fit_stan2.Rda"))
load(file = paste0(analysis_path,"fit_stan2.Rda"))
print(fit_stan2)
traceplot(fit_stan2, inc_warmup = FALSE)

# STAN model2 - both contingency levels: simulate posterior data [NOT WORKING YET!]----
params = c()
for (chain in 1:4){
  params = rbind(params,as.data.frame(extract(fit_stan2, permuted=FALSE, inc_warmup = FALSE)[,chain,]))
}
chains_length <- length(params$lp__)
N_subjects = stan_data$N_subjects
N_trials = stan_data$N_trials

simulations = data.frame(Sub = rep(1:N_subjects,rep(N_trials,N_subjects)),
                         Trial = rep(1:N_trials,N_subjects),
                         Run = as.vector(stan_data$Run),
                         Cue = as.vector(stan_data$Cue), 
                         Contingency = as.vector(stan_data$Contingency), 
                         RT = as.vector(stan_data$RT)
)

simulations = simulations[simulations$RT<=10000,]
n_sims = nrow(simulations)
simulations$sim_i = sample(1:chains_length,n_sims,replace = TRUE)
simulations$theta_b0 = stan_data$theta_b0
simulations$theta_b1 = NA
for (sub_i in 1:N_subjects){
  theta_b1_varname = paste0("theta_b1_rand[",sub_i,"]")
  theta_b2_varname = paste0("theta_b2_rand[",sub_i,"]")
  mu1_b0_varname = paste0("mu1_rand[",sub_i,"]")
  mu2_b0_varname = "mu2_fix"
  
  simulations$theta_b1[simulations$Sub == sub_i] = params[simulations$sim_i[simulations$Sub == sub_i],theta_b1_varname]
  simulations$theta_b2[simulations$Sub == sub_i] = params[simulations$sim_i[simulations$Sub == sub_i],theta_b2_varname]
  simulations$mu_1[simulations$Sub == sub_i] = params[simulations$sim_i[simulations$Sub == sub_i],mu1_b0_varname]
  simulations$mu_2[simulations$Sub == sub_i] = params[simulations$sim_i[simulations$Sub == sub_i],mu2_b0_varname]
  
  cat("\rprogress:",round(100*sub_i/N_subjects),"%")
  flush.console() 
}
simulations$Contingency2 = as.factor(simulations$Contingency)
levels(simulations$Contingency2) = c("50%", "100%")
simulations$theta = simulations$theta_b0 + (simulations$theta_b1*simulations$Contingency + (1-simulations$Contingency)*simulations$theta_b2)*simulations$Run
simulations$p = pnorm(simulations$theta)
simulations$mu_2 = params$mu2_fix[simulations$sim_i]
simulations$sigma_1 = params$`sigma_e[1]`[simulations$sim_i]
simulations$sigma_2 = params$`sigma_e[2]`[simulations$sim_i]
simulations$selection = rbinom(n_sims,1,simulations$p)
simulations$RT_simulation = rnorm(n_sims, simulations$mu_1 ,simulations$sigma_1) * simulations$selection +
  rnorm(n_sims, simulations$mu_2 ,simulations$sigma_2) * (1-simulations$selection)
hist(simulations$RT_simulation - simulations$Cue,100)

simulations$RT_simulation_eff = simulations$RT_simulation - simulations$Cue
simulations$RT_eff = simulations$RT - simulations$Cue
simulations$Run2 = as.factor(simulations$Run*19 + 1)
simulations$selection_fac= as.factor(simulations$selection)

# Actual Data
ggplot(data = simulations) +
  geom_density(aes(x =  RT,y = ..density.., color = Run2)) + 
  facet_grid(. ~ Contingency2)
# Posterior Distribution
ggplot(data = simulations) +
  geom_density(aes(x =  RT_simulation,y = ..density.., color = Run2)) + 
  facet_grid(. ~ Contingency2)

ggplot(data = simulations) + geom_density(aes(x = RT, y = ..density..,color = 'Original Data')) + 
  geom_density(aes(x = RT_simulation, y = ..density..,color = 'Posterior Simulation'))

ggplot(data = simulations) + geom_density(aes(x = RT, y = ..density..,color = 'Original Data')) + 
  geom_density(aes(x = RT_simulation, y = ..density..,color = 'Posterior Simulation')) + 
  facet_grid(. ~ Contingency2)

ggplot(data = simulations) + geom_density(aes(x = RT_eff, y = ..density..,color = 'Original Data')) + 
  geom_density(aes(x = RT_simulation_eff, y = ..density..,color = 'Posterior Simulation')) + 
  scale_x_continuous(name = "RT - from cue onset") + 
  facet_grid(. ~ Contingency2)


# Simulated posterior - split by Gaussians: count
ggplot() +
  geom_density(data = subset(simulations, selection ==0), aes(x =  RT_simulation,y = ..count.., color = Run2)) + 
  geom_density(data = subset(simulations, selection ==1), aes(x =  RT_simulation,y = ..count.., color = Run2))

ggplot() +
  geom_density(data = subset(simulations, selection ==0), aes(x =  RT_eff,y = ..count.., color = Run2)) 
ggplot() +
  geom_density(data = subset(simulations, selection ==1), aes(x =  RT_eff,y = ..count.., color = Run2))

# STAN model2 - both contingency levels: correlation with choice ----

model_summary = as.data.frame(summary(fit_stan2)$summary)
theta1_mean = model_summary$mean[grep("theta_b1_rand",row.names(model_summary))]
theta2_mean = model_summary$mean[grep("theta_b2_rand",row.names(model_summary))]
mu1_mean = model_summary$mean[grep("mu1_rand",row.names(model_summary))]

filelist=c()
for (s in subjects){
  filelist=c(filelist,Sys.glob(paste(path, "BM_",s,"_Probe_*.txt",sep="")))
}
Probe_data=c()
for (f in filelist){
  Probe_data=rbind(Probe_data,read.table(f,header=T,na.strings=c(999,999000)))
}

probe_dat = Probe_data
probe_dat$sub_i2 = as.numeric(factor(probe_dat$subjectID))
probe_mean = tapply(probe_dat$Outcome, list(probe_dat$sub_i2, probe_dat$Contingency), mean, na.rm = T)
DM_dat = melt(probe_mean, varnames = c("sub","Contingency"), value.name = "probe_effect")
DM_dat$theta = c(theta2_mean,theta1_mean)
DM_dat$Contingency2 = as.factor(DM_dat$Contingency)
levels(DM_dat$Contingency2) = c("50% Contingency", "100% Contingency")
ggplot(data = DM_dat, aes(x = theta, y = probe_effect, color = Contingency2)) +
  labs(x = expression(paste(theta," slope")), y = 'proportion of trials Go stimuli were chosen', color = "Contingency") +
  geom_smooth(method = 'lm',se = TRUE, fullrange=TRUE) +
  geom_point() +
  theme_bw() + 
  theme(text = element_text(size=18), legend.position = "none") + 
  geom_abline(slope = 0, intercept = .5, linetype = 2) +
  facet_grid(. ~ Contingency2) + 
  scale_y_continuous(breaks = seq(0,1,0.1), limits = c(0,1), expand = c(0,0))

summary(lm.beta(lm(probe_effect ~ theta, data = subset(DM_dat,Contingency==0.5))))
summary(lm.beta(lm(probe_effect ~ theta, data = subset(DM_dat,Contingency==1))))
summary(lmer(probe_effect ~ theta + Contingency2 + (1 |sub), data = subset(DM_dat)))
summary(lmer(probe_effect ~ Contingency2 + (1 |sub), data = subset(DM_dat)))


with(data =DM_dat, tapply(theta, Contingency,mean))
summary(lmer(scale(theta) ~ Contingency2 + (1|sub), data = subset(DM_dat)))
summary(lmer(theta ~ Contingency2 + (1|sub), data = subset(DM_dat)))

DM_dat$theta_diff = c(theta1_mean-theta2_mean,theta1_mean-theta2_mean)
ggplot(data = DM_dat, aes(x = theta_diff, y = probe_effect, color = Contingency2)) +
  labs(x = expression(paste(theta," slope")), y = 'proportion of trials Go stimuli were chosen') +
  geom_smooth(method = 'lm',se = FALSE) +
  geom_point()
summary(lm.beta(lm(probe_effect ~ theta_diff, data = subset(DM_dat,Contingency==1))))
summary(lmer(theta ~ 1 + Contingency2 + (1|sub), data = DM_dat))


ggplot(data = DM_dat, aes(x = theta_diff, y = theta, color = Contingency2)) +
  labs(x = expression(paste(theta," diff")), y = 'Theta') +
  geom_smooth(method = 'lm',se = FALSE) +
  geom_point()


# NEW ----

# STAN model - individual items - only 100%  contingency: data organization ====
stan_file = "training_mixture_individual_items_100Contingency.stan"

subs2test = 1:100 # set to smaller number to examine less participants for speed
dat = subset(CAT_data_GO, (sub_i %in% subs2test) & Contingency == 1)
N_trials_per_sub = dat %>% group_by(sub_i) %>% summarise(n()) %>% pull
N_trials_max = max(N_trials_per_sub)
N_runs = max(dat$Run)
N_subjects = nlevels(factor(dat$sub_i))
N_stim = dat %>% filter(sub_i ==1, Contingency==1) %>% pull(StimNum) %>% as.factor() %>% nlevels()
RT_table = matrix(data = 999000,nrow = N_trials_max,ncol = N_subjects)
Run_table = RT_table
Stim_table = RT_table
N_trials_valid = c()
for (n in 1:N_subjects){
  # find values
  RT_tmp = dat$RT[dat$sub_i == n]
  Run_tmp = dat$Run[dat$sub_i == n]
  Stim_tmp = dat$StimNum[dat$sub_i == n]
  
  # scale values
  Run_tmp = (Run_tmp-min(Run_tmp))/max(Run_tmp-min(Run_tmp))
  Stim_tmp = as.numeric(as.factor(Stim_tmp))
  # organize together
  N_trials_valid[n] = sum(dat$sub_i == n)
  RT_table[1:N_trials_valid[n],n] = RT_tmp
  Run_table[1:N_trials_valid[n],n] = Run_tmp 
  Stim_table[1:N_trials_valid[n],n] = Stim_tmp 
}

stan_data = list(
  Run = Run_table,
  Cue = 850,
  theta_b0 = -3.1, # fix the proportion of early onset at pnorm(theta_b0) at the first run
  RT = RT_table,
  Stim = Stim_table,
  N_trials = N_trials_max,
  N_trials_valid = N_trials_valid,
  N_runs = N_runs,
  N_subjects = N_subjects,
  N_stim = N_stim
)

# STAN model - individual items - only 100%  contingency: write model ----

write("// Stan model mixture gaus model - individual items - only 100% contingency
      data {
      int N_subjects;
      int N_trials;
      int N_trials_valid[N_subjects];
      int N_stim;
      real Cue;
      real theta_b0;
      real RT[N_trials, N_subjects];
      real Run[N_trials, N_subjects];
      int Stim[N_trials, N_subjects];
      }
      
      parameters {
      // Fixed effect: mu1(Anticipatory), mu2(Cue dependent), theta1(100% contingency), theta2(50% contingency), sigma_e1, sigma_e2
      real<lower=0, upper=Cue+100> mu1_fix; // Anticipatory mean
      real<lower=Cue> mu2_fix; // Cue dependent mean
      real theta_fix[1]; // theta0 + slope (for run)
      real<lower=0> sigma_e[2]; // error terms
      
      // SD for random effect: mu1, theta1, theta2
      // real<lower=0> mu1_sd[1]; // SD of the random effects around the fixed mean 
      real<lower=0> theta_sub_sd[1]; // SD of the random effects around the fixed thetas
      real<lower=0> theta_stim_sd[1]; // SD of the random effects around the fixed thetas
      
      // Random effect parameter: mu1, theta1, theta2
      // real <upper=Cue+100> mu1_rand[N_subjects]; // random intercept for early onest Gaussian's mean
      // real theta_b0_rand[N_subjects]; // random intercept for theta - fix at -2.5
      real theta_1_sub[N_subjects]; // random intercept for theta
      // real theta_2_sub[N_subjects]; // random intercept for theta
      real theta_1_sub_stim[N_subjects,N_stim]; // random intercept for theta
      // real theta_2_sub_stim[N_subjects,N_stim]; // random intercept for theta
      }
      
      model {
      //priors
      mu1_fix ~ normal(500,500);
      mu2_fix ~ normal(1000,500);
      theta_fix ~ normal(0,0.7);
      sigma_e ~ normal(0,1000);
      theta_sub_sd ~ cauchy(0,1);
      theta_stim_sd ~ cauchy(0,1);
      // mu1_sd ~ normal(0,150);

      
      //likelihood
      theta_1_sub ~ normal(theta_fix[1],theta_sub_sd[1]); // 100% contingency
      for (s in 1:N_subjects){
      // mu1_rand[s] ~ normal(mu1_fix,mu1_sd);
      // theta_b0_rand[s] ~ normal(theta_fix[1],theta_sub[1]); fix at -2.5
      theta_1_sub_stim[s,] ~ normal(theta_1_sub[s],theta_stim_sd[1]); // 100% contingency

      for (t in 1:N_trials_valid[s]){
      int stim_i;
      stim_i = Stim[t,s]; 

      target += log_mix(Phi_approx(theta_b0 + theta_1_sub_stim[s,stim_i]*Run[t,s]),
      normal_lpdf(RT[t,s] | mu1_fix, sigma_e[1]),
      normal_lpdf(RT[t,s] | mu2_fix, sigma_e[2]));
      }
      }
      }", 
      stan_file)

# STAN model - individual items - only 100%  contingency: run ----

# fit_stan3 <- stan(file = stan_file, data = stan_data, iter = 4000, chains = 4, seed = 1, control = list(adapt_delta =.85, max_treedepth=14))
# save(list = ("fit_stan3","stan_data"), file = paste0(analysis_path,"fit_stan3.Rda"))
load(file = paste0(analysis_path,"fit_stan3.Rda"))
print(fit_stan3)
traceplot(fit_stan3, inc_warmup = FALSE)
params = c()
for (chain in 1:4){
  params = rbind(params,as.data.frame(rstan::extract(fit_stan3, permuted=FALSE, inc_warmup = FALSE)[,chain,]))
}
pairs(fit_stan3, pars= c(colnames(params)[1:6],"lp__"))

# STAN model - individual items - only 100%  contingency: simulate posterior data (NOT READY) ----
chains_length <- length(params$lp__)
N_subjects = stan_data$N_subjects
N_trials = stan_data$N_trials

simulations = data.frame(Sub = rep(1:N_subjects,rep(N_trials,N_subjects)),
                         Trial = rep(1:N_trials,N_subjects),
                         Run = as.vector(stan_data$Run),
                         Cue = as.vector(stan_data$Cue), 
                         Stim = as.vector(stan_data$Stim),
                         #Contingency = as.vector(stan_data$Contingency), 
                         RT = as.vector(stan_data$RT)
)

simulations = simulations[simulations$RT<=10000,]
n_sims = nrow(simulations)
simulations$sim_i = sample(1:chains_length,n_sims,replace = TRUE)
simulations$theta_b0 = stan_data$theta_b0
simulations$theta_b1 = NA
for (sub_i in 1:N_subjects){
  for (stim_i in 1:N_stim){
    theta_b1_varname = paste0("theta_1_sub_stim[",sub_i,",",stim_i,"]")
    #mu1_b0_varname = paste0("mu1_rand[",sub_i,"]")
    
    sim_ind = which(simulations$Sub == sub_i & simulations$Stim == stim_i)
    simulations$theta_b1[sim_ind] = params[simulations$sim_i[sim_ind],theta_b1_varname]
    
    cat("\rprogress:",round(100*sub_i/N_subjects),"%")
    flush.console() 
  }
}
#simulations$Contingency2 = as.factor(simulations$Contingency)
#levels(simulations$Contingency2) = c("50%", "100%")
#simulations$theta = simulations$theta_b0 + (simulations$theta_b1*simulations$Contingency + (1-simulations$Contingency)*simulations$theta_b2)*simulations$Run
simulations$theta = simulations$theta_b0 + simulations$theta_b1*simulations$Run

simulations$p = pnorm(simulations$theta)
simulations$mu_1 = params$mu1_fix[simulations$sim_i]
simulations$mu_2 = params$mu2_fix[simulations$sim_i]
simulations$sigma_1 = params$`sigma_e[1]`[simulations$sim_i]
simulations$sigma_2 = params$`sigma_e[2]`[simulations$sim_i]
simulations$selection = rbinom(n_sims,1,simulations$p)
simulations$RT_simulation = rnorm(n_sims, simulations$mu_1 ,simulations$sigma_1) * simulations$selection +
  rnorm(n_sims, simulations$mu_2 ,simulations$sigma_2) * (1-simulations$selection)
hist(simulations$RT_simulation - simulations$Cue,100)

simulations$RT_simulation_eff = simulations$RT_simulation - simulations$Cue
simulations$RT_eff = simulations$RT - simulations$Cue
simulations$Run2 = as.factor(simulations$Run*19 + 1)
simulations$selection_fac= as.factor(simulations$selection)

# Actual Data
ggplot(data = simulations) +
  geom_density(aes(x =  RT,y = ..density.., color = Run2))  
# Posterior Distribution
ggplot(data = simulations) +
  geom_density(aes(x =  RT_simulation,y = ..density.., color = Run2)) 

ggplot(data = simulations) + geom_density(aes(x = RT, y = ..density..,color = 'Original Data')) + 
  geom_density(aes(x = RT_simulation, y = ..density..,color = 'Posterior Simulation')) + 
  facet_grid(. ~ Contingency2)

ggplot(data = simulations) + geom_density(aes(x = RT_eff, y = ..density..,color = 'Original Data')) + 
  geom_density(aes(x = RT_simulation_eff, y = ..density..,color = 'Posterior Simulation')) + 
  scale_x_continuous(name = "RT - from cue onset") + 
  facet_grid(. ~ Contingency2)


# Simulated posterior - split by Gaussians: count
ggplot() +
  geom_density(data = subset(simulations, selection ==0), aes(x =  RT_simulation,y = ..count.., color = Run2)) + 
  geom_density(data = subset(simulations, selection ==1), aes(x =  RT_simulation,y = ..count.., color = Run2))

ggplot() +
  geom_density(data = subset(simulations, selection ==0), aes(x =  RT_eff,y = ..count.., color = Run2)) 
ggplot() +
  geom_density(data = subset(simulations, selection ==1), aes(x =  RT_eff,y = ..count.., color = Run2))

# STAN model - individual items - only 100%  contingency: correlation with choice ----

thetas = params %>% select(contains(c("theta_1_sub_stim"))) %>% summarise_all(mean) %>%
  gather(sub_stim_i2, Theta) %>% mutate(sub_stim_i = gsub(pattern = "theta_1_sub_stim|\\[|\\]", replacement = "", x=sub_stim_i2)
  )
Probe_data2 = Probe_data %>% filter(Contingency==1) %>% mutate(sub_i = as.numeric(factor(subjectID)))
Probe_data2$stim_go = ifelse(Probe_data2$IsleftGo,as.character(Probe_data2$ImageLeft),as.character(Probe_data2$ImageRight))
Probe_data2$stim_go_i = NA

for (n in 1:N_subjects){
  # find values
  Stim_tmp = Probe_data2$stim_go[Probe_data2$sub_i == n]
  Stim_tmp2 = as.numeric(as.factor(as.character(Stim_tmp)))
  Probe_data2$stim_go_i[Probe_data2$sub_i == n] = Stim_tmp2
}

Probe_data2 = Probe_data2 %>% mutate(sub_stim_i=interaction(sub_i,stim_go_i,sep = ",")) %>%
  left_join(thetas, by = "sub_stim_i")


DM_dat = Probe_data2 %>% filter(!is.na(Theta))
ggplot(data = DM_dat, aes(x = Theta, y = Outcome)) + 
  geom_jitter(width = 0.5, height = 0.01, alpha=0.1) +
  stat_smooth(method="glm", method.args=list(family="binomial"), se=FALSE)

ggplot(data = DM_dat, aes(x = Theta, y = Outcome, color = subjectID)) + 
  geom_jitter(width = 0.5, height = 0.01, alpha=0.1) +
  stat_smooth(method="glm", method.args=list(family="binomial"), se=FALSE)

ggplot(data = DM_dat, aes(x = Theta, y = Outcome)) + 
  geom_jitter(width = 0.5, height = 0.01, alpha=0.1) +
  stat_smooth(method="glm", method.args=list(family="binomial"), se=FALSE) + 
  facet_wrap(. ~ subjectID, nrow = 3)

summary(glmer(Outcome ~ 1 + Theta + (1 + Theta|subjectID), data = DM_dat, family = "binomial"))
summary(lmer(Outcome ~ 1 + Theta + (1 + Theta|subjectID), data = DM_dat))

DM_dat2 = DM_dat %>% group_by(sub_i) %>% summarise(M_theta = mean(Theta), Probe_effect = mean(Outcome, na.rm=T))
ggplot(data = DM_dat2, aes(x = M_theta, y = Probe_effect)) + 
  geom_point() +
  #stat_smooth(method="glm", se=FALSE)
  stat_smooth(method="glm", method.args=list(family="binomial"), se=FALSE)

Probe_data_100Cont = Probe_data2 

# STAN model - individual items - only 50%  contingency: data organization ====
stan_file = "training_mixture_individual_items_50Contingency.stan"

subs2test = 1:100 # set to smaller number to examine less participants for speed
dat = subset(CAT_data_GO, (sub_i %in% subs2test) & Contingency == 0.5)
N_trials_per_sub = dat %>% group_by(sub_i) %>% summarise(n()) %>% pull
N_trials_max = max(N_trials_per_sub)
N_runs = max(dat$Run)
N_subjects = nlevels(factor(dat$sub_i))
N_stim = dat %>% filter(sub_i ==1, Contingency==0.5) %>% pull(StimNum) %>% as.factor() %>% nlevels()
RT_table = matrix(data = 999000,nrow = N_trials_max,ncol = N_subjects)
Run_table = RT_table
Stim_table = RT_table
N_trials_valid = c()
for (n in 1:N_subjects){
  # find values
  RT_tmp = dat$RT[dat$sub_i == n]
  Run_tmp = dat$Run[dat$sub_i == n]
  Stim_tmp = dat$StimNum[dat$sub_i == n]
  
  # scale values
  Run_tmp = (Run_tmp-min(Run_tmp))/max(Run_tmp-min(Run_tmp))
  Stim_tmp = as.numeric(as.factor(Stim_tmp))
  # organize together
  N_trials_valid[n] = sum(dat$sub_i == n)
  RT_table[1:N_trials_valid[n],n] = RT_tmp
  Run_table[1:N_trials_valid[n],n] = Run_tmp 
  Stim_table[1:N_trials_valid[n],n] = Stim_tmp 
}

stan_data = list(
  Run = Run_table,
  Cue = 850,
  theta_b0 = -3.1, # fix the proportion of early onset at pnorm(theta_b0) at the first run
  RT = RT_table,
  Stim = Stim_table,
  N_trials = N_trials_max,
  N_trials_valid = N_trials_valid,
  N_runs = N_runs,
  N_subjects = N_subjects,
  N_stim = N_stim
)

# STAN model - individual items - only 50%  contingency: write model ----

write("// Stan model mixture gaus model - individual items - only 50% contingency
        data {
        int N_subjects;
        int N_trials;
        int N_trials_valid[N_subjects];
        int N_stim;
        real Cue;
        real theta_b0;
        real RT[N_trials, N_subjects];
        real Run[N_trials, N_subjects];
        int Stim[N_trials, N_subjects];
        }
        
        parameters {
        // Fixed effect: mu1(Anticipatory), mu2(Cue dependent), theta1(100% contingency), theta2(50% contingency), sigma_e1, sigma_e2
        real<lower=0, upper=Cue+100> mu1_fix; // Anticipatory mean
        real<lower=Cue> mu2_fix; // Cue dependent mean
        real theta_fix[1]; // theta0 + slope (for run)
        real<lower=0> sigma_e[2]; // error terms
        
        // SD for random effect: mu1, theta1, theta2
        // real<lower=0> mu1_sd[1]; // SD of the random effects around the fixed mean 
        real<lower=0> theta_sub_sd[1]; // SD of the random effects around the fixed thetas
        real<lower=0> theta_stim_sd[1]; // SD of the random effects around the fixed thetas
        
        // Random effect parameter: mu1, theta1, theta2
        // real <upper=Cue+100> mu1_rand[N_subjects]; // random intercept for early onest Gaussian's mean
        // real theta_b0_rand[N_subjects]; // random intercept for theta - fix at -2.5
        real theta_1_sub[N_subjects]; // random intercept for theta
        // real theta_2_sub[N_subjects]; // random intercept for theta
        real theta_1_sub_stim[N_subjects,N_stim]; // random intercept for theta
        // real theta_2_sub_stim[N_subjects,N_stim]; // random intercept for theta
        }
        
        model {
        //priors
        mu1_fix ~ normal(500,500);
        mu2_fix ~ normal(1000,500);
        theta_fix ~ normal(0,0.7);
        sigma_e ~ normal(0,1000);
        theta_sub_sd ~ cauchy(0,1);
        theta_stim_sd ~ cauchy(0,1);
        // mu1_sd ~ normal(0,150);
        
        
        //likelihood
        theta_1_sub ~ normal(theta_fix[1],theta_sub_sd[1]); // 100% contingency
        for (s in 1:N_subjects){
        // mu1_rand[s] ~ normal(mu1_fix,mu1_sd);
        // theta_b0_rand[s] ~ normal(theta_fix[1],theta_sub[1]); fix at -2.5
        theta_1_sub_stim[s,] ~ normal(theta_1_sub[s],theta_stim_sd[1]); // 100% contingency
        
        for (t in 1:N_trials_valid[s]){
        int stim_i;
        stim_i = Stim[t,s]; 
        
        target += log_mix(Phi_approx(theta_b0 + theta_1_sub_stim[s,stim_i]*Run[t,s]),
        normal_lpdf(RT[t,s] | mu1_fix, sigma_e[1]),
        normal_lpdf(RT[t,s] | mu2_fix, sigma_e[2]));
        }
        }
        }", 
      stan_file)

# STAN model - individual items - only 50%  contingency: run ----

# fit_stan4 <- stan(file = stan_file, data = stan_data, iter = 4000, chains = 4, seed = 1, control = list(adapt_delta =.85, max_treedepth=14))
# save(fit_stan4, file = paste0(analysis_path,"fit_stan4.Rda"))
load(file = paste0(analysis_path,"fit_stan4.Rda"))
print(fit_stan4)
traceplot(fit_stan4, inc_warmup = FALSE)
params = c()
for (chain in 1:4){
  params = rbind(params,as.data.frame(rstan::extract(fit_stan4, permuted=FALSE, inc_warmup = FALSE)[,chain,]))
}
pairs(fit_stan3, pars= c(colnames(params)[1:6],"lp__"))

# STAN model - individual items - only 50%  contingency: simulate posterior data (NOT READY) ----
chains_length <- length(params$lp__)
N_subjects = stan_data$N_subjects
N_trials = stan_data$N_trials

simulations = data.frame(Sub = rep(1:N_subjects,rep(N_trials,N_subjects)),
                         Trial = rep(1:N_trials,N_subjects),
                         Run = as.vector(stan_data$Run),
                         Cue = as.vector(stan_data$Cue), 
                         Stim = as.vector(stan_data$Stim),
                         #Contingency = as.vector(stan_data$Contingency), 
                         RT = as.vector(stan_data$RT)
)

simulations = simulations[simulations$RT<=10000,]
n_sims = nrow(simulations)
simulations$sim_i = sample(1:chains_length,n_sims,replace = TRUE)
simulations$theta_b0 = stan_data$theta_b0
simulations$theta_b1 = NA
for (sub_i in 1:N_subjects){
  for (stim_i in 1:N_stim){
    theta_b1_varname = paste0("theta_1_sub_stim[",sub_i,",",stim_i,"]")
    #mu1_b0_varname = paste0("mu1_rand[",sub_i,"]")
    
    sim_ind = which(simulations$Sub == sub_i & simulations$Stim == stim_i)
    simulations$theta_b1[sim_ind] = params[simulations$sim_i[sim_ind],theta_b1_varname]
    
    cat("\rprogress:",round(100*sub_i/N_subjects),"%")
    flush.console() 
  }
}
#simulations$Contingency2 = as.factor(simulations$Contingency)
#levels(simulations$Contingency2) = c("50%", "100%")
#simulations$theta = simulations$theta_b0 + (simulations$theta_b1*simulations$Contingency + (1-simulations$Contingency)*simulations$theta_b2)*simulations$Run
simulations$theta = simulations$theta_b0 + simulations$theta_b1*simulations$Run

simulations$p = pnorm(simulations$theta)
simulations$mu_1 = params$mu1_fix[simulations$sim_i]
simulations$mu_2 = params$mu2_fix[simulations$sim_i]
simulations$sigma_1 = params$`sigma_e[1]`[simulations$sim_i]
simulations$sigma_2 = params$`sigma_e[2]`[simulations$sim_i]
simulations$selection = rbinom(n_sims,1,simulations$p)
simulations$RT_simulation = rnorm(n_sims, simulations$mu_1 ,simulations$sigma_1) * simulations$selection +
  rnorm(n_sims, simulations$mu_2 ,simulations$sigma_2) * (1-simulations$selection)
hist(simulations$RT_simulation - simulations$Cue,100)

simulations$RT_simulation_eff = simulations$RT_simulation - simulations$Cue
simulations$RT_eff = simulations$RT - simulations$Cue
simulations$Run2 = as.factor(simulations$Run*19 + 1)
simulations$selection_fac= as.factor(simulations$selection)

# Actual Data
ggplot(data = simulations) +
  geom_density(aes(x =  RT,y = ..density.., color = Run2))  
# Posterior Distribution
ggplot(data = simulations) +
  geom_density(aes(x =  RT_simulation,y = ..density.., color = Run2)) 

ggplot(data = simulations) + geom_density(aes(x = RT, y = ..density..,color = 'Original Data')) + 
  geom_density(aes(x = RT_simulation, y = ..density..,color = 'Posterior Simulation')) + 
  facet_grid(. ~ Contingency2)

ggplot(data = simulations) + geom_density(aes(x = RT_eff, y = ..density..,color = 'Original Data')) + 
  geom_density(aes(x = RT_simulation_eff, y = ..density..,color = 'Posterior Simulation')) + 
  scale_x_continuous(name = "RT - from cue onset") + 
  facet_grid(. ~ Contingency2)


# Simulated posterior - split by Gaussians: count
ggplot() +
  geom_density(data = subset(simulations, selection ==0), aes(x =  RT_simulation,y = ..count.., color = Run2)) + 
  geom_density(data = subset(simulations, selection ==1), aes(x =  RT_simulation,y = ..count.., color = Run2))

ggplot() +
  geom_density(data = subset(simulations, selection ==0), aes(x =  RT_eff,y = ..count.., color = Run2)) 
ggplot() +
  geom_density(data = subset(simulations, selection ==1), aes(x =  RT_eff,y = ..count.., color = Run2))

# STAN model - individual items - only 50%  contingency: correlation with choice ----

thetas = params %>% select(contains(c("theta_1_sub_stim"))) %>% summarise_all(mean) %>%
  gather(sub_stim_i2, Theta) %>% mutate(sub_stim_i = gsub(pattern = "theta_1_sub_stim|\\[|\\]", replacement = "", x=sub_stim_i2)
  )
Probe_data2 = Probe_data %>% filter(Contingency==0.5) %>% mutate(sub_i = as.numeric(factor(subjectID)))
Probe_data2$stim_go = ifelse(Probe_data2$IsleftGo,as.character(Probe_data2$ImageLeft),as.character(Probe_data2$ImageRight))
Probe_data2$stim_go_i = NA

for (n in 1:N_subjects){
  # find values
  Stim_tmp = Probe_data2$stim_go[Probe_data2$sub_i == n]
  Stim_tmp2 = as.numeric(as.factor(as.character(Stim_tmp)))
  Probe_data2$stim_go_i[Probe_data2$sub_i == n] = Stim_tmp2
}

Probe_data2 = Probe_data2 %>% mutate(sub_stim_i=interaction(sub_i,stim_go_i,sep = ",")) %>%
  left_join(thetas, by = "sub_stim_i")


DM_dat = Probe_data2 %>% filter(!is.na(Theta))
ggplot(data = DM_dat, aes(x = Theta, y = Outcome)) + 
  geom_jitter(width = 0.5, height = 0.01, alpha=0.1) +
  stat_smooth(method="glm", method.args=list(family="binomial"), se=FALSE)

ggplot(data = DM_dat, aes(x = Theta, y = Outcome, color = subjectID)) + 
  geom_jitter(width = 0.5, height = 0.01, alpha=0.1) +
  stat_smooth(method="glm", method.args=list(family="binomial"), se=FALSE)

ggplot(data = DM_dat, aes(x = Theta, y = Outcome)) + 
  geom_jitter(width = 0.5, height = 0.01, alpha=0.1) +
  stat_smooth(method="glm", method.args=list(family="binomial"), se=FALSE) + 
  facet_wrap(. ~ subjectID, nrow = 3)

summary(glmer(Outcome ~ 1 + Theta + (1 + Theta|subjectID), data = DM_dat, family = "binomial"))
summary(lmer(Outcome ~ 1 + Theta + (1 + Theta|subjectID), data = DM_dat))

DM_dat2 = DM_dat %>% group_by(sub_i) %>% summarise(M_theta = mean(Theta), Probe_effect = mean(Outcome, na.rm=T))
ggplot(data = DM_dat2, aes(x = M_theta, y = Probe_effect)) + 
  geom_point() +
  #stat_smooth(method="glm", se=FALSE)
  stat_smooth(method="glm", method.args=list(family="binomial"), se=FALSE)

Probe_data_50Cont = Probe_data2 

# Probe analysis: merge 100% Contingency and 50% Contingency model =====

Probe_data_merged = rbind(Probe_data_100Cont, Probe_data_50Cont)
Probe_data_merged$Contingency2 = factor(Probe_data_merged$Contingency, labels =  c("50% Contingency", "100% Contingency"))
DM_dat3 = Probe_data_merged %>% group_by(sub_i, Contingency2) %>% summarise(M_theta = mean(Theta), Probe_effect = mean(Outcome, na.rm=T))

ggplot(data = DM_dat3, aes(x = M_theta, y = Probe_effect, color = Contingency2)) + 
  geom_point(alpha =0.3) +
  stat_smooth(method="glm", method.args=list(family="binomial"), se=FALSE, fullrange = TRUE) +
  theme_bw() +
  geom_hline(yintercept = 0.5, linetype=2) + 
  scale_y_continuous(limits = c(0,1), expand = c(0,0)) + 
  labs(x = expression(paste(theta," slope (mean of participant)")), y = 'Proportion of trials Go stimuli were chosen') 

ggplot(data = Probe_data_merged, aes(x = Theta, y = Outcome, color = Contingency2)) + 
  geom_jitter(width = 0.01, height = 0.03, alpha=0.05) +
  stat_smooth(method="glm", method.args=list(family="binomial"), se=FALSE, fullrange = TRUE) +
  theme_bw() +
  geom_hline(yintercept = 0.5, linetype=2) + 
  labs(x = expression(paste(theta," slope (per individual item")), y = 'Were Go stimuli chosen') 


