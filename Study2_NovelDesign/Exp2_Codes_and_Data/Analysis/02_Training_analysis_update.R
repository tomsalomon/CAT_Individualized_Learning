...
CAT_data_GO = CAT_data_GO %>% mutate(anticipatory = RT <= (CueOnset + 100))
ggplot(CAT_data_GO, aes(x=Run, y=as.numeric(anticipatory), color = Contingency2)) + geom_jitter(width = 0.5, height = 0.01, alpha=0.1) + 
  stat_smooth(method="glm", method.args=list(family="binomial"), se=FALSE, formula = 'y ~ x')

ggplot(CAT_data_GO, aes(x=Run, y=as.numeric(anticipatory), color = Contingency2)) + 
  facet_wrap(. ~ Subject) + 
...
# STAN model - individual items (NOT WORKING YET!): write model ----
...
# STAN model - individual items (NOT WORKING YET!): run ----

fit_stan_test <- stan(file = stan_file, data = stan_data, iter = 500, chains = 4, seed = 1, control = list(adapt_delta =.8))
save(fit_stan2, file = paste0(analysis_path,"fit_stan2.Rda"))
load(file = paste0(analysis_path,"fit_stan2.Rda"))
print(fit_stan2)
traceplot(fit_stan2, inc_warmup = FALSE)

# STAN model - individual items (NOT WORKING YET!): simulate posterior data ----
params = c()
for (chain in 1:4){
  params = rbind(params,as.data.frame(rstan::extract(fit_stan2, permuted=FALSE, inc_warmup = FALSE)[,chain,]))
}
...
# STAN model - individual items (NOT WORKING YET!): correlation with choice ----
params %>% select(contains(c("mu1_rand"))) %>% summarise_all(mean) %>%
  gather(Sub, Mu1, contains("mu1")) 
