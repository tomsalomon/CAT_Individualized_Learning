...
data2analyze$scan = as.factor(data2analyze$scan)
...
with(data2analyze,tapply(gaze_go,list(scan),mean,na.rm=T))
with(data2analyze,tapply(gaze_nogo,list(scan),mean,na.rm=T))
...
dev.off()
#Data_by_subscan_go = aggregate(gaze_go  ~ subjectID  + scan + PairType, data = data2analyze, FUN =  mean, na.rm = TRUE)
#Data_by_subscan_nogo = aggregate(gaze_nogo ~ subjectID  + scan + PairType, data = data2analyze, FUN =  mean, na.rm = TRUE)
Data_by_subscan_go = aggregate(gaze_go  ~ subjectID  + scan, data = data2analyze, FUN =  mean, na.rm = TRUE)
Data_by_subscan_nogo = aggregate(gaze_nogo ~ subjectID  + scan, data = data2analyze, FUN =  mean, na.rm = TRUE)
...
if (sessionNum ==1) {
  print("Scan 1 - before CAT")
  summary(lm(gaze_diff ~ 1 , data = subset(Data_by_subscan, scan == 1 )))
  
  print("Scan 2 - after CAT")
  summary(lm(gaze_diff ~ 1 , data = subset(Data_by_subscan, scan == 2 )))
  
  print("Time effect")
  summary(lmer(gaze_diff ~ 1 + scan + (1 |subjectID), data = Data_by_subscan))
} else if (sessionNum ==2) {
  print("Scan 2 - Follow-up")
  summary(lm(gaze_diff ~ 1 , data = subset(Data_by_subscan, scan == 3 )))
}
